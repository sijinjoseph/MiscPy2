...
def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    ...
    for row in range(len(road)):
            for col in range(len(road[0])):
                if road[row][col] > fastest_speed:
                    fastest_speed = road[row][col]
                
    def distance(x1, y1, x2, y2):
        return abs(y2-y1) * (1.0/fastest_speed) + abs(x2-x1) * lane_change_cost
    
    
    D = [[[max_value, max_value, False]] * len(road[0]) for _ in range(len(road))]
    D[init[0]][init[1]] = [0, 0, False]
    
    num_nodes_checked = 0
    while True: 
        min_heurestic = max_value
        min_row = -1
        min_col = -1 
        for row in range(len(D)):
            for col in range(len(D[0])):
                if D[row][col][2] == False and D[row][col][0] < min_heurestic:
                    min_row, min_col, min_heurestic = row, col, D[row][col][0]
        
        heuristic_cost, actual_cost, current_row, current_col = D[min_row][min_col][0], D[min_row][min_col][1], min_row, min_col
        D[current_row][current_col][2] = True
        num_nodes_checked += 1
        if current_row == goal[0] and current_col == goal[1]:
            show(D)
            print '# nodes checked = ', num_nodes_checked
            return actual_cost        
        else:
            moves = []
            if current_col < len(road[0]) - 1:
                if current_row > 0:
                    moves.append([-1, 1, lane_change_cost])
                    
                if current_row < len(road) - 1:
                    moves.append([1, 1, lane_change_cost])
            ...
            for move in moves:   
                ...
                if road[new_row][new_col] == 0:
                    continue
                
                new_cost = actual_cost + (1.0/road[new_row][new_col]) + move[2]
                new_heurestic = new_cost + distance(current_row, current_col, goal[0], goal[1])
                if D[new_row][new_col][2] == False and new_heurestic < D[new_row][new_col][0]:
                    D[new_row][new_col] = [new_heurestic, new_cost, False]
