...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        # update the information maxtrix/vector based on the robot motion
        data_to_map = list(range(dim+2))
        data_to_map.remove(2)
        data_to_map.remove(3)
        
        Omega = Omega.expand(dim + 2, dim + 2, data_to_map, data_to_map)
        Xi = Xi.expand(dim + 2, 1, data_to_map, [0])
        ...
        XiP = Xi.take(range(2, dim + 2), range(1))
        A = Omega.take(range(2), range(2, dim + 2))
        B = Omega.take(range(2), range(2))
        C = Xi.take(range(2), range(1))
