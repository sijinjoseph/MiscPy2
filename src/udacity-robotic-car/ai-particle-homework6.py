...
class robot:
    ...
    def move(self, motion):
        ...
        if beta <= 0.001:
            result.set(self.x + d * cos(self.orientation), self.y + d * sin(self.orientation), self.orientation)
        else:        
            R = d / beta
            cx = self.x - sin(self.orientation) * R
            cy = self.y + cos(self.orientation) * R
            result.set(cx + sin(self.orientation + beta) * R, cy - cos(self.orientation + beta) * R, (self.orientation + beta) % (2*pi))
    ...
    def sense(self, use_noise=1):
        noise = random.gauss(0, self.bearing_noise) if use_noise == 1 else 0
        Z = [(atan2(lmk[0] - self.y, lmk[1] - self.x) % (2 *pi)) - self.orientation + noise for lmk in landmarks]
        return Z
